...
def abrir_janela():
    janela = tk.Toplevel()
    janela.title("AgroManager - Gerenciar Clientes")
    ...
    ttk.Label(top_inner, text="🏢 Gerenciar Clientes", font=("Arial", 14, "bold"), background='#1a5f7a', foreground='white').pack()
    ...
    tabela = ttk.Treeview(frame_list, columns=('ID', 'Nome', 'Tipo', 'Contato', 'Produto', 'Volume'), height=20, show='headings')
    tabela.column('#0', width=0, stretch=tk.NO)
    tabela.column('ID', anchor=tk.CENTER, width=40)
    tabela.column('Nome', anchor=tk.W, width=80)
    tabela.column('Tipo', anchor=tk.W, width=80)
    tabela.column('Contato', anchor=tk.W, width=80)
    tabela.column('Produto', anchor=tk.W, width=80)
    tabela.column('Volume', anchor=tk.CENTER, width=60)
    ...
    tabela.heading('Volume', text='Volume')
    ...
    def atualizar_lista():
        ...
        for item in dados:
            tabela.insert('', tk.END, values=(item['id'], item['nome'], item['tipo'], item['contato'], item['produto'], item['volume']))
    ...
    tk.Label(frame_form, text="Tipo:", bg='#ecf0f1', fg='#333333', font=('Arial', 10, 'bold')).grid(row=2, column=0, sticky=tk.W, pady=5, padx=5)
    ...
    tk.Label(frame_form, text="Produto Comprado:", bg='#ecf0f1', fg='#333333', font=('Arial', 10, 'bold')).grid(row=4, column=0, sticky=tk.W, pady=5, padx=5)
    produto_entry = tk.Entry(frame_form, font=('Arial', 10), bg='white', fg='#333333')
    produto_entry.grid(row=4, column=1, sticky=tk.EW, pady=5, padx=5)

    tk.Label(frame_form, text="Volume:", bg='#ecf0f1', fg='#333333', font=('Arial', 10, 'bold')).grid(row=5, column=0, sticky=tk.W, pady=5, padx=5)
    volume_entry = tk.Entry(frame_form, font=('Arial', 10), bg='white', fg='#333333')
    volume_entry.grid(row=5, column=1, sticky=tk.EW, pady=5, padx=5)
    ...
    def selecionar(event=None):
        ...
        produto_entry.insert(0, valores[4])
        volume_entry.delete(0, tk.END)
        volume_entry.insert(0, valores[5])
    ...
    def criar():
        ...
        salvar_dados(dados)
        atualizar_lista()
        messagebox.showinfo("Cliente criado", f"Cliente '{novo.get('nome','')}' (ID: {novo.get('id','')}) criado com sucesso.")
    ...
    def atualizar():
        ...
        for item in dados:
            if item["id"] == id_entry.get():
                ...
                salvar_dados(dados)
                atualizar_lista()
                messagebox.showinfo("Cliente atualizado", f"Cliente '{item.get('nome','')}' (ID: {item.get('id','')}) atualizado com sucesso.")
                return
            messagebox.showerror("Erro - Atualizar", f"ID '{id_entry.get()}' não encontrado.")

    def deletar():
        if not messagebox.askyesno("Confirmar remoção", f"Deseja realmente deletar o cliente com ID '{id_entry.get()}'?"):
            return
        dados = carregar_dados()
        dados = [item for item in dados if item["id"] != id_entry.get()]
        salvar_dados(dados)
        atualizar_lista()
        messagebox.showinfo("Cliente deletado", f"Cliente com ID '{id_entry.get()}' deletado com sucesso.")
