...
def abrir_janela():
    janela = tk.Toplevel()
    janela.title("AgroManager - Gerenciar Funcionários")
    ...
    ttk.Label(top_inner, text="👥 Gerenciar Funcionários", font=("Arial", 14, "bold"), background='#1a5f7a', foreground='white').pack()
    ...
    tabela = ttk.Treeview(frame_list, columns=('ID', 'Nome', 'Cargo', 'Contato', 'Turno', 'Especialidade'), height=20, show='headings')
    tabela.column('#0', width=0, stretch=tk.NO)
    tabela.column('ID', anchor=tk.CENTER, width=40)
    tabela.column('Nome', anchor=tk.W, width=80)
    tabela.column('Cargo', anchor=tk.W, width=80)
    tabela.column('Contato', anchor=tk.W, width=70)
    tabela.column('Turno', anchor=tk.W, width=60)
    tabela.column('Especialidade', anchor=tk.W, width=80)
    ...
    def atualizar_lista():
        ...
        for item in dados:
            tabela.insert('', tk.END, values=(item['id'], item['nome'], item['cargo'], item['contato'], item['turno'], item['especialidade']))
    ...
    tk.Label(frame_form, text="Cargo:", bg='#ecf0f1', fg='#333333', font=('Arial', 10, 'bold')).grid(row=2, column=0, sticky=tk.W, pady=5, padx=5)
    cargo_entry = tk.Entry(frame_form, font=('Arial', 10), bg='white', fg='#333333')
    cargo_entry.grid(row=2, column=1, sticky=tk.EW, pady=5, padx=5)

    tk.Label(frame_form, text="Contato:", bg='#ecf0f1', fg='#333333', font=('Arial', 10, 'bold')).grid(row=3, column=0, sticky=tk.W, pady=5, padx=5)
    contato_entry = tk.Entry(frame_form, font=('Arial', 10), bg='white', fg='#333333')
    contato_entry.grid(row=3, column=1, sticky=tk.EW, pady=5, padx=5)

    tk.Label(frame_form, text="Turno:", bg='#ecf0f1', fg='#333333', font=('Arial', 10, 'bold')).grid(row=4, column=0, sticky=tk.W, pady=5, padx=5)
    turno_entry = tk.Entry(frame_form, font=('Arial', 10), bg='white', fg='#333333')
    turno_entry.grid(row=4, column=1, sticky=tk.EW, pady=5, padx=5)

    tk.Label(frame_form, text="Especialidade:", bg='#ecf0f1', fg='#333333', font=('Arial', 10, 'bold')).grid(row=5, column=0, sticky=tk.W, pady=5, padx=5)
    especialidade_entry = tk.Entry(frame_form, font=('Arial', 10), bg='white', fg='#333333')
    especialidade_entry.grid(row=5, column=1, sticky=tk.EW, pady=5, padx=5)
    ...
    def selecionar(event=None):
        ...
        cargo_entry.insert(0, valores[2])
        contato_entry.delete(0, tk.END)
        contato_entry.insert(0, valores[3])
        turno_entry.delete(0, tk.END)
        turno_entry.insert(0, valores[4])
        especialidade_entry.delete(0, tk.END)
        especialidade_entry.insert(0, valores[5])
    ...
    def criar():
        ...
        salvar_dados(dados)
        atualizar_lista()
        messagebox.showinfo("Funcionário criado", f"Funcionário '{novo.get('nome','')}' (ID: {novo.get('id','')}) criado com sucesso.")
    ...
    def atualizar():
        ...
        for item in dados:
            if item["id"] == id_entry.get():
                ...
                salvar_dados(dados)
                atualizar_lista()
                messagebox.showinfo("Funcionário atualizado", f"Funcionário '{item.get('nome','')}' (ID: {item.get('id','')}) atualizado com sucesso.")
                return
            messagebox.showerror("Erro - Atualizar", f"ID '{id_entry.get()}' não encontrado.")

    def deletar():
        if not messagebox.askyesno("Confirmar remoção", f"Deseja realmente deletar o funcionário com ID '{id_entry.get()}'?"):
            return
        dados = carregar_dados()
        dados = [item for item in dados if item["id"] != id_entry.get()]
        salvar_dados(dados)
        atualizar_lista()
        messagebox.showinfo("Funcionário deletado", f"Funcionário com ID '{id_entry.get()}' deletado com sucesso.")
