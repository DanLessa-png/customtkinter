...
def abrir_janela():
    janela = tk.Toplevel()
    janela.title("AgroManager - Gerenciar Máquinas Agrícolas")
    ...
    ttk.Label(top_inner, text="🚜 Gerenciar Máquinas", font=("Arial", 14, "bold"), background='#1a5f7a', foreground='white').pack()
    ...
    tabela = ttk.Treeview(frame_list, columns=('ID', 'Tipo', 'Modelo', 'Status'), height=20, show='headings')
    tabela.column('#0', width=0, stretch=tk.NO)
    tabela.column('ID', anchor=tk.CENTER, width=50)
    tabela.column('Tipo', anchor=tk.W, width=100)
    tabela.column('Modelo', anchor=tk.W, width=100)
    tabela.column('Status', anchor=tk.W, width=100)
    ...
    def atualizar_lista():
        ...
        for item in dados:
            tabela.insert('', tk.END, values=(item['id'], item['tipo'], item['modelo'], item['status']))
    ...
    tk.Label(frame_form, text="Tipo:", bg='#ecf0f1', fg='#333333', font=('Arial', 10, 'bold')).grid(row=1, column=0, sticky=tk.W, pady=5, padx=5)
    tipo_entry = tk.Entry(frame_form, font=('Arial', 10), bg='white', fg='#333333')
    tipo_entry.grid(row=1, column=1, sticky=tk.EW, pady=5, padx=5)

    tk.Label(frame_form, text="Modelo:", bg='#ecf0f1', fg='#333333', font=('Arial', 10, 'bold')).grid(row=2, column=0, sticky=tk.W, pady=5, padx=5)
    modelo_entry = tk.Entry(frame_form, font=('Arial', 10), bg='white', fg='#333333')
    modelo_entry.grid(row=2, column=1, sticky=tk.EW, pady=5, padx=5)

    tk.Label(frame_form, text="Status:", bg='#ecf0f1', fg='#333333', font=('Arial', 10, 'bold')).grid(row=3, column=0, sticky=tk.W, pady=5, padx=5)
    status_entry = tk.Entry(frame_form, font=('Arial', 10), bg='white', fg='#333333')
    status_entry.grid(row=3, column=1, sticky=tk.EW, pady=5, padx=5)
    ...
    def selecionar(event=None):
        ...
        tipo_entry.insert(0, valores[1])
        modelo_entry.delete(0, tk.END)
        modelo_entry.insert(0, valores[2])
        status_entry.delete(0, tk.END)
        status_entry.insert(0, valores[3])
    ...
    def criar():
        ...
        salvar_dados(dados)
        atualizar_lista()
        messagebox.showinfo("Máquina criada", f"Máquina '{novo.get('modelo','')}' (ID: {novo.get('id','')}) criada com sucesso.")
    ...
    def atualizar():
        ...
        for item in dados:
            if item["id"] == id_entry.get():
                ...
                salvar_dados(dados)
                atualizar_lista()
                messagebox.showinfo("Máquina atualizada", f"Máquina '{item.get('modelo','')}' (ID: {item.get('id','')}) atualizada com sucesso.")
                return
            messagebox.showerror("Erro - Atualizar", f"ID '{id_entry.get()}' não encontrado.")

    def deletar():
        if not messagebox.askyesno("Confirmar remoção", f"Deseja realmente deletar a máquina com ID '{id_entry.get()}'?"):
            return
        dados = carregar_dados()
        dados = [item for item in dados if item["id"] != id_entry.get()]
        salvar_dados(dados)
        atualizar_lista()
        messagebox.showinfo("Máquina deletada", f"Máquina com ID '{id_entry.get()}' deletada com sucesso.")
